...
try {
  ...
  // Keywords that trigger patch version increment
  const patchKeywords = ['fix', 'hotfix', 'patch', 'chore', 'build', 'docs'];
  ...
  // Generate version string with build metadata (semver-compliant)
  const version = `v0.${minor}.${patch}+${commitHash}`;
...
`;
  ...
  console.log(`✓ Generated version: ${version} (${minor} feature commits, ${patch} patch commits)`);
} catch (error) {
